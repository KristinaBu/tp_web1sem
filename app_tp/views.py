...
def index(request):
    ...
    return render(request,'app_tp/index.html',
                  {'questions' : paginate(QUETIONS, request, 5),
                   'tags' : TAGS})

def hot(request):
    '''один вопрос под своим id'''
    return render(request,'app_tp/index.html', {'questions' : paginate(HOT_QUETIONS, request, 5),
                                                'tags' : TAGS})

def tag(request, tag_id):
    tag_item = TAGS[tag_id]
    question_item = QUETIONS[1]
    return render(request,'app_tp/tag.html', {'tag' : tag_item, 'questions' : paginate(QUETIONS, request, 5),
...
def question(request, question_id):
    ...
    return render(request,'app_tp/question.html', {'question' : question_item,
                                                   'tags' : TAGS,
...
def ask(request):
    return render(request,'app_tp/ask.html', {'tags' : TAGS})


def login(request):
    return render(request,'app_tp/login.html', {'tags' : TAGS})


def signup(request):
    return render(request,'app_tp/signup.html', {'tags' : TAGS})
